...
#define PIPSLITE_WRAPPER_VERSION "* epson-escpr-wrapepr2 is a part of " PACKAGE_STRING
...
/* attach point */
	...
	while (1)
	{
		...
		if (pfp == NULL)
		{
			...
			sprintf (tmpbuf, "%s/%s \"%s\" %d %d %d %s %s %s %s %s %s %s %s %s %s %s %s",	//
#else																					//
...
#ifdef GCOMSW_PRIVATE										// RHV2
	strcpy(filter_opt_p->compression , "0");
	strcpy(filter_opt_p->line , "0");
	strcpy(filter_opt_p->colorspace , "0");

	attr = ppdFindAttr(ppd_p, "ESCPRCompression", NULL);//
	if(attr){
		tok = strtok( attr->value, ",");
		if( tok != NULL ){
			strcpy(filter_opt_p->compression , tok);
			tok = strtok( NULL, "," );

			if( tok != NULL ){
				strcpy(filter_opt_p->line , tok);
				tok = strtok( NULL, "," );

				if( tok != NULL ){
					strcpy(filter_opt_p->colorspace , tok);
				}	
			}
		}
	}
