...
#define PIPSLITE_FILTER_USAGE "Usage: $ epson-escpr model width_pixel height_pixel Ink PageSize MediaType Duplex InputSlot Brightness Contrast Saturation Compression line colorspace"
...
EPS_ERR_CODE epsInitJob(filter_option_t *fopt){	// Lib5.6
...
//	printer->pmData.state = EPS_PM_STATE_NOT_FILTERED;
	...
	printer->protocol = EPS_PROTOCOL_LPR;

	char *modelName = fopt->model;														// Lib5.6
	if(strlen(modelName) >= 6)															//
		if(memcmp(modelName, "Epson ", 6) == 0 || memcmp(modelName, "EPSON ", 6) == 0)	//
			modelName += 6;																//
	strncpy(printer->modelName, modelName, EPS_NAME_BUFFSIZE - 1);						//
	debug_msg("model name = %s\n", printer->modelName);									//
	obsSetPrinter(printer);																//

#ifdef GCOMSW_PRIVATE															// RHV2
		printer->supprotedComplessMode = (EPS_UINT16)atoi(fopt->compression);	//
		if(printer->supprotedComplessMode & EPS_SP_COMPMODE_RHV2){				//
			printer->rhv2BandHeight = (EPS_INT32)atoi(fopt->line);				//
			if(printer->rhv2BandHeight == 0)									//
				printer->rhv2BandHeight = 16;									//

			printer->supprotedColorSpace = (EPS_INT32)atoi(fopt->colorspace);	//
		}																		//
	debug_msg("comp mode = %d\n", printer->supprotedComplessMode);				//
	debug_msg("rhv2 band height = %d\n", printer->rhv2BandHeight);				//
	debug_msg("supproted color space = %d\n", printer->supprotedColorSpace);	//
#endif																			//
...
//				return 0;
...
#ifdef GCOMSW_PRIVATE								// RHV2
	strncpy (fopt.compression, argv[14], NAME_MAX);	//
	strncpy (fopt.line, argv[15], NAME_MAX);	//
	strncpy (fopt.colorspace, argv[16], NAME_MAX);	//
#endif												//
	...
	if(jobAttr.mediaTypeIdx == PM_MTID_CDDVD || jobAttr.mediaTypeIdx == PM_MTID_CDDVDHIGH || jobAttr.mediaTypeIdx == PM_MTID_CDDVDGLOSSY){	// CDLabel
		jobAttr.printLayout = EPS_MLID_CDLABEL;												//
		jobAttr.cdDimIn = 18;																//
		jobAttr.cdDimOut = 120;																//
	}																						//
...
#ifdef GCOMSW_PRIVATE														// RHV2
	if(printJob.printer->supprotedComplessMode & EPS_SP_COMPMODE_RHV2){		//
		debug_msg("begin epscmdRhv2Init\n");								//
		err = epscmdRhv2Init(&sendDataBuf, &sendDataBufSize);				//
		if(err){															//
			debug_msg("Error occurred in \"epscmdRhv2Init\": %d\n", err);	//
			err_fatal("Error occurred in \"epscmdRhv2Init\".");				//
		}else																//
			debug_msg ("epscmdRhv2Init success\n");							//
	}else																	//
#endif																		//
	epsInitVariable();

//	pageInitFunctions();	// Lib5.6
...
//	printJob.printer = &curPrinter;
	...
	if (err){
		...
		err_fatal ("Error occurred in \"SendStartJob\".");	/* exit */
	}else{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	if(jobAttr.printLayout == EPS_MLID_CDLABEL){	// CDLabel
		x_mag = 1.0;								//
		y_mag = 1.0;								//
	}												//
...
#if (HAVE_PPM)
		int k; FILE *fp; char ppmname[64];
		sprintf(ppmname, "/tmp/print%03d.ppm", page_count);
		fp = fopen(ppmname, "w");
...
#if (HAVE_PPM)
					fp = fopen(ppmname, "a");
					for(k = 0; k < bytes_per_line / byte_par_pixel * 3; k++)
						fprintf(fp, "%u ", (unsigned char)bandBmp.bits[k]);
					fprintf(fp, "\n");
					fclose(fp);
#endif
...
set_pips_parameter (filter_option_t *filter_opt_p, EPS_OPT *printOpt)
{
	...
	debug_msg("print quality = %d\n", jobAttr.printQuality);
